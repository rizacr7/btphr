...
class Proses extends CI_Controller {
	...
	function prosesppu(){
        $this->load->view('general/header');
        $this->load->view('general/sidebar');
        $this->load->view('proses/proses_ppu');
        $this->load->view('general/footer');
    }
	...
	function proses_gajipegawai(){
		...
		if($rdt > 0){
			if($bulan == $blnnow && $tahun == $thnnow){
				...
				//---delete approval sdm ---
				$queryHapus = "DELETE FROM approval_sdm WHERE bulan = $bulan AND tahun = $tahun AND kd_jenis = 1";
				$this->db->query($queryHapus);

				$data = array(
					'kd_jenis' => 1,
					'nm_dokumen' => "GAJI PEGAWAI",
					'bulan' => $bulan,
					'tahun' => $tahun,
					'tgl_proses' => date("Y-m-d"),
					);
				$this->db->insert('approval_sdm',$data);
	...
	function tab_gajipegawai(){
        ...
        $data = $this->proses_model->get_data_gaji("", "", "", 0, 0,$param);
